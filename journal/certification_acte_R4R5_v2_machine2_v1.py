#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""CERTIFICATION DE L'ACTE DELTA nn (R4/R5) **v2** -- machine 2, v1, 11/09/2026.

FEUILLE NEUVE, ET NON UNE EDITION DE CELLE DU LOT v1 : cette derniere est manifestee
au canon 4e7dafb54a908f71 ; l'editer perimerait son ancre. Meme corps, trois attentes
mises a jour (les trois correctifs H1, H2, H3) et une quatrieme ajoutee (H4).
...
ACTE = os.path.join(RAC, "entrant_machine1_2026-09-11_acte_R4R5_v2", "lot",
...
chk("A-4", "[H3] la borne annoncee est 1e-5 et elle est JUSTE",
    ecg < 1e-5 and cite("a 1e-5 pres") and cite("9.5e-06") and not cite("a 1e-6 pres"),
    "max ecart relatif %.2e" % ecg)
...
chk("A-6", "[H2] la fourchette 4.7 a 5.7 contient les TROIS ecarts",
    min(ec6) >= 4.65 and max(ec6) <= 5.75 and cite("BAS de 4.7 a 5.7 pour cent")
    and cite("5.42, 5.73, 4.66") and not cite("BAS de 5.4 a 5.7"),
...
chk("A-7", "[H1] la serie est DECLAREE non homogene en resolution",
    cite("la serie n'est PAS homogene en") and cite("a la resolution")
    and cite("non rejoues") and cite("-1.050 sur") and cite("plus ou moins 0.01")
    and not cite("pente ln P / ln eps = -1.071 sur quatre points : la periode"),
    "0.320 et 0.2808 declares a la resolution d'origine ; pente -1.050 sur les "
    "deux resolus donnee a cote")
# H4 : l'outil de la regle (ii)
chk("nn.6", "[H4] la regle (ii) porte desormais son OUTIL",
    cite("avec son OUTIL") and cite("TOUS ses points sont a la meme resolution")
    and cite("s'ecrit dans la feuille, pas dans la prose"))
