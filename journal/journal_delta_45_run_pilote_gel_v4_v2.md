...
HISTORIQUE DU DELTA
  ...
  Certification du gel v4 (f10ffcf3, 01/08) : CERTIFIE, bf9866a7 autorise le
      code. Deux formulations de la v1 de ce delta y sont prises en defaut
      (sections 5a et 5b de la certification) ; elles sont corrigees ici.
  v2. Corrige 45.3 (le point qui exclut ; les ilots, chacun avec SON
      domaine) ; consigne la certification du gel v4, l'ECART DECLARE a la
      lettre de la clause de v3, les chiffres de plan P(m < 3) et
      l'espacement des treize ; PRE-DECLARE la paire G8 du programme M12,
...
-----------------------------------------------------------------
  (corrige en v2 sur les deux points releves par la certification du gel
  v4, sections 5a et 5b)
  ...
  fenetre grossiere.
  LE POINT QUI EXCLUT (formulation v1 corrigee -- elle suggerait un
  artefact de bord de fenetre, la donnee dit l'inverse et elle dit mieux) :
  le point explosif est le DERNIER des 165 points de la fenetre grossiere,
  tous sous le seuil 0.98 s* ; les 164 autres sont PROPRES. Sous 0.90 s*,
  au pas de 0.005 s*, RIEN n'explose : le mouchetage commence a 0.90 s* et
  non ailleurs. L'exclusion est un fait de la LIGNE, pas de la decoupe de
  l'instrument. (La coincidence exacte 0.5298467113 = 0.900000 s* est celle
  du point de grille partage par les deux fenetres.) L'exclusion est a
  3.6e14 epsilons du bord 0.98 : les temoins S4/S6 l'etablissent SUR DONNEE
  -- zero ligne ou l'indice 40 compte sous le seuil, zero explosif a cet
  indice, sur 24 balayages.
...
45.6 PRE-DECLARATION, AVANT TOUTE LIGNE DE CODE : LA PAIRE G8 DE M12
---------------------------------------------------------------------
  Le programme fige porte "G8 (regression p=4) : 2" sans nommer les deux
  points -- meme classe que G1' avant sa correction S5a. PRE-DECLARE ICI,
  regle mecanique et aveugle aux donnees :
      G8a/G8b portent sur les RANGS 1 ET 13 de la liste (2.22 et 1.86) --
      le premier et le dernier du programme.
  MOTIFS : (i) le rang 1 (2.22) est aussi l'ancre de G2, dont le compte
  "3 x 2 = 6" exige les DEUX signes a p=4 en ce point -- la paire G8 y
  fournit la recherche sgn = -1 qui rend le compte coherent ; (ii) le rang
  13 (1.86) couvre la zone gauche, la plus exposee a l'attrition, ou la
  regression de parite est la plus informative. A CONTRESIGNER par machine
  2 a la certification du script ; si elle prefere une autre paire
  mecanique, la presente declaration etablit seulement que le choix est
  anterieur au code et aveugle aux mesures.
...
(663b17e2), G1' au bit contre ed0e27b1, paire G8 pre-declaree (45.6),
selftest qui mord, pre-vol joue par la machine qui detient les sources --
puis le run : 75 recherches, et la premiere lecture de E de toute la
campagne.
