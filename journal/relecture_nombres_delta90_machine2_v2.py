#!/usr/bin/env python3
# -*- coding: ascii -*-
"""RELECTURE DES NOMBRES DE L'ACTE delta 90 -- AUX SOURCES. machine 2, v2, 12/09/2026.

v2 = v1 (0aae5d85df39862b, non editee) + l'acte est un ARGUMENT + UNE SOURCE ABSENTE OU NON
AUTHENTIFIEE REND NON JOUE ET LA FEUILLE CONTINUE (remarque de machine 1 : la v1 s'arretait sur
la premiere piece non detenue, 23 controles non rejoues sans le dire) : chaque controle qui lit
cette source est declare NON JOUE, compte a part, et le bilan les nomme. Les controles sont ceux
de la v1 ; ceux qui lisaient une source directement passent par `lit()`, qui rend NON JOUE si la
source est absente.

L'acte cite des nombres. Cette feuille de MESURE les relit A LA SOURCE, en DEUX JAMBES :
...
lecture. Les presences se testent sur BLANCS NORMALISES des deux cotes (regle 4 du 28/08).
...
if len(sys.argv) != 2:
    sys.exit('usage : relecture_nombres_delta90_machine2_v2.py <acte journal_delta_nn_..._vN.md>')
...
class Absente(str):
    """Une source absente : toute lecture rend NON JOUE."""


ABSENT = Absente('')
NJ = object()   # condition d'un controle NON JOUE


def chk(n, c, d=''):
    if c is NJ:
        NON_JOUES.append(n)
        print('  [NON J] %-56s %s' % (n[:56], 'source absente'))
        return
    OK.append((n, bool(c)))
...
def lit(*sources):
    """Rend une fonction qui evalue une condition, ou NJ si l'une des sources est absente."""
    def _(cond):
        return NJ if any(isinstance(s, Absente) or s is None for s in sources) else cond()
    return _
...
def source(etq, chemin, attendu):
    """Authentifie une source par canon AVANT de la lire ; absente ou non authentifiee : NON JOUE."""
    p = P(*chemin)
    if attendu is None or not os.path.exists(p) or canon(p) != attendu:
        NON_JOUES.append('source %s' % etq)
        print('  [NON J] source %-49s %s' % (etq[:49], ('ABSENTE : ' if not os.path.exists(p) else 'canon %s != %s : ' % (canon(p), attendu)) + os.path.join(*chemin)[-45:]))
        return ABSENT
...
def js(etq, chemin, attendu):
    p = P(*chemin)
    if not os.path.exists(p) or canon(p) != attendu:
        NON_JOUES.append('source %s' % etq)
        print('  [NON J] source %-49s %s' % (etq[:49], ('ABSENTE : ' if not os.path.exists(p) else 'canon %s != %s : ' % (canon(p), attendu)) + os.path.join(*chemin)[-45:]))
        return None
    chk('source %-28s %s' % (etq, attendu), canon(p) == attendu, os.path.join(*chemin)[-60:])
...
chk('les deux logs sont IDENTIQUES (CRLF normalise)', lit(LOG3, LOG3_M1)(lambda: LOG3 == LOG3_M1), '%d lignes' % LOG3.count('\n'))
...
chk('0.589 (acte) est l arrondi du 0.5886 du log', lit(LOG3)(lambda: '0.5886' in LOG3 and '0.589' in ACTE and '%.3f' % 0.5886 == '0.589'), '0.589')
chk('l acte dit 15 cellules, le log en compte 15 (3 m x 5 kT)',
    lit(LOG3)(lambda: '15 cellules' in ACTE and len(re.findall(r'^\s+\d\.\d\d\s+\|', LOG3, re.M)) == 5), '5 lignes kT x 3 colonnes m')
...
if None in (J_MOI, J_NOY, J_LIB):
    for n_ in ('porteur et six cellules (18 controles)', 'porte et reglage (11 controles)'):
        chk(n_, NJ)
else:
    ...
    chk('TROIS a la resolution du journal', len(dif3) == 3 and 'TROIS a la resolution du journal' in ACTE_N, ','.join(dif3))
    ...
    chk('separation nette : 3 grossieres, 3 fines, rien entre 1e-07 et 1e-02', len(gros) == 3 and len(fin) == 3
        and 'aucune entre 1e-07 et 1e-02' in ACTE_N, '%s | %s' % (','.join(gros), ','.join(fin)))
    ...
    chk('x_b_num differe sur 7 des 9', len(xb) == 7 and 'differe sur 7 des 9' in ACTE_N, ','.join(xb))
...
chk('le pre-vol m1 du 28/08 rend la meme branche 4 (des deux cotes)', lit(LM1)(lambda: 'branche 4' in LM1 and 'des deux cotes' in ACTE), 'branche 4')
J_REF = js('run_temoin_delta85 (registre)', ('registre', 'runs', 'run_temoin_delta85', 'resultats_temoin.json'), '644240dc894c2733')
if J_REF is None:
    chk('tol_int deposee = valeur machine 2', NJ)
else:
...
cite('77 == LIBM, 0 == NOYAU', '77 / 82', src=L3, dans_acte='LIBM sur 77 et son run NOYAU sur 0')
chk('0 / 82 dans le log', lit(L3)(lambda: '0 / 82' in L3), '0 / 82')
cite('14 sur 774 hors exposees', '14 / 774', src=L3, dans_acte='14 feuilles sur 774')
...
chk('91 et 154 CR', lit(L28)(lambda: '91 CR' in L28 and '154 CR' in L28 and '91 et 154 CR' in ACTE), '91, 154')
cite('19/19', '19/19', src=L28)
chk('0 texte sur 771 .md', lit(L28)(lambda: '0 texte' in L28 and '771 .md' in L28 and '0 sur 771 .md' in ACTE), '0 / 771')
...
chk('5 appels a condition constante sur 7 feuilles', lit(LG)(lambda: '5 appel(s) chk a condition constante sur 7 feuille(s)' in LG
    and '5 appels a condition constante sur 7' in ACTE_N), '5 / 7')
if isinstance(LG, Absente):
    chk('les trois lignes de D-ACA-7', NJ)
    chk('le False de la l.69 de verification_28b est sous condition', NJ)
else:
...
for etq, s_log, s_acte in (('91 .py', '91 fichiers .py', '91 fichiers .py'), ('545 puissances', 'TOTAL        545', '545 puissances'),
                           ('232 exposables', 'EXPOSABLE    232', '232 EXPOSABLES'),
    ...
    chk('%-32s %s' % (etq, s_log), lit(LR)(lambda: s_log in LR and s_acte in ACTE_N), s_log)
...
if isinstance(LV8, Absente):
    chk('42 / 9 / 6 sur le v8', NJ)
    chk('57 puissances sur le v8', NJ)
else:
    cl = {t[0]: int(t[1]) for t in (l.split() for l in LV8.split('\n')) if len(t) == 2 and t[0] in ('EXPOSABLE', 'CARRE', 'CONST') and t[1].isdigit()}
    cite('42 / 9 / 6 sur le v8', '42 EXPOSABLES, 9 CARRE, 6 CONST', recalc=(cl.get('EXPOSABLE'), cl.get('CARRE'), cl.get('CONST')), attendu=(42, 9, 6))
...
chk('36 exposables au v3 depose', lit(N25)(lambda: 'banc_qualification_machine1 v3 36' in N25 and '36 au v3' in ACTE), '36')
chk('207 .md au registre, 0 clause', lit(N25)(lambda: '207 .md' in N25 and '0 sur 207' in ACTE), '207')
...
for etq, s_src, s_acte in (('158/158', '158 + absentes 0 + ecarts 0 == 158', '158/158'), ('driver 15/15', '15/15', '15/15'),
    ...
    chk('%-32s %s' % (etq, s_acte), lit(MREJ)(lambda: s_src in MREJ and s_acte in ACTE_N), s_src)
...
chk('numpy 2.4.4 sur machine 1', lit(NC)(lambda: 'numpy 2.4.4' in NC and '2.4.4' in ACTE), '2.4.4')
...
for etq, s_src, s_acte in (('245 au run', '**245**', '245'), ('238 identiques', 'PREDITES IDENTIQUES ....... 238', '238 predites identiques'),
    ...
    chk('%-32s %s' % (etq, s_acte), lit(N70)(lambda: s_src in N70 and s_acte in ACTE_N), s_src)
...
chk('96 controles, 95 passent, 1 mord (v4), P-A-1', lit(C4)(lambda: '96 controles, 95 passent, 1 mord' in C4 and 'P-A-1' in C4
...
chk('VERDICT CERTIFIEE (v8), selftest 103/103', lit(V8)(lambda: 'VERDICT : CERTIFIEE.' in V8 and '103/103' in V8
...
chk('VERDICT CERTIFIEE (v11), gel a2e7ef3e237c5acf', lit(T11)(lambda: 'CERTIFIEE' in T11 and 'a2e7ef3e237c5acf' in T11 and 'GEL DU VOLET T' in ACTE), 'v11')
LV5 = source('note livraison v5 (m1)', ('note_machine1_livraison_banc_v5_v1.md',), 'be633ae91ae295b5')
chk('53 remplacements v3 -> v4, 24 remplacements Q01..Q24', lit(LV5)(lambda: '53 rempl' in LV5 and '24 remplacements' in LV5 and 'Q01..Q24' in LV5
    and '53 remplacements' in ACTE_N and 'Q01..Q24' in ACTE), '53 / 24')
LS = source('surface d exposition du registre .log', ('G3_reponse', 'surface_exposition_registre_machine2_v1.log'), '9c707ac4260c3e0a')
chk('34 logs deposes, 0 niveau de dispatch', lit(LS)(lambda: 'niveau de dispatch     : 0' in LS and '34' in LS and '34 logs deposes' in ACTE), '34 / 0')
S29 = source('SUIVI 29a (m1)', ('SUIVI_campagne_2026-08-29a.md',), 'd0747b01700e4ad8')
chk('neuf defauts D-I-1..9 tous leves (SUIVI 29a)', lit(S29)(lambda: 'D-I-1..9 tous LEVES' in S29 and 'Neuf defauts D-I-1 a D-I-9 ont ete leves' in ACTE_N), 'D-I-1..9')
...
dets = re.findall(r'\b[0-9a-f]{16}\b', bloc('  Detention unique, declaree (H1)', 'nn.2 LA CHAINE'))
chk('H1 : 18 empreintes a detention unique, toutes citees ailleurs dans l acte', len(dets) == 18
    and all(ACTE.count(t) >= 2 for t in dets) and '18 des 95 empreintes' in ACTE, '%d' % len(dets))
raw = open(ACTE_P, 'rb').read()
larg = max(len(l) for l in raw.split(b'\n'))
chk('l acte est ASCII, LF, newline final, largeur <= 90 (le 89 va a 129)', all(b < 128 for b in raw)
...
print('BILAN RELECTURE : %d/%d controles PASSENT ; %d NON JOUE(S)%s'
      % (k, n, len(NON_JOUES), (' : ' + ' ; '.join(x[:40] for x in NON_JOUES)) if NON_JOUES else ''))
print('=====================================================================')
