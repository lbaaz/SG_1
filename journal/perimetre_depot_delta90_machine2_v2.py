#!/usr/bin/env python3
# -*- coding: ascii -*-
"""LE PERIMETRE DU DEPOT delta 90 -- ENUMERE DEPUIS LES CITATIONS DE L'ACTE. machine 2, v2,
12/09/2026.

v2 = v1 (9a3e72664e4a13c2, non editee) + l'acte est un ARGUMENT (le nom de la piece de depot en
...
if len(sys.argv) != 3:
    sys.exit('usage : perimetre_depot_delta90_machine2_v2.py <clone frais du registre> <acte journal_delta_nn_..._vN.md>')
...
SORTIES = {os.path.normcase(MAN), os.path.normcase(os.path.join(ICI, 'perimetre_depot_delta90_machine2_v2.log'))}
VALEURS = {
...
ea = emp_de(ACTE)
lignes.insert(0, ('journal/journal_delta_%s_constante_A_v%s.md' % (NUMERO, VERSION), ea[0], ea[1], ea[2], ea[3],
                  rel(ACTE, RAC), 'cet acte, copie de %s' % os.path.basename(ACTE)))
for f in ('relecture_nombres_delta90_machine2_v2.py', 'relecture_nombres_delta90_machine2_v2.log',
          'perimetre_depot_delta90_machine2_v2.py'):
...
corps += ("Piece de depot : journal_delta_%s_constante_A_v%s.md, COPIE OCTET POUR OCTET de "
