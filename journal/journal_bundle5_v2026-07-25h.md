...
*Version du fichier : 25/07-h (gel MANCHE M5). Même nom de
...
ERRATA E1–E14, pré-enregistrements M2/M3/M4/M5.*
...
## MANCHE M5 — PRE-ENREGISTREMENT (gele avant l'ecriture du code)

**Question (issue de §16)** : pourquoi la route unique (2,1) fixe-t-elle le
PLAFOND CLASSIQUE seulement a p=3 ? Hypothese testee, **H-COURSE** : a w2=2,
deux canaux de destruction sont en course le long de l'amplitude — la couche
resonante (2,1) et la dominance directe (g x^(p-1) ecrase le rappel lineaire).
Le cubique serait resonance-limite ; p>=4 dominance-limite.

**Methode : chirurgie MONOMIALE en espace des modes.** EOM canoniques
(derivees en seance, verifiees) : xdd1 = -w1^2 x1 + (dV/dx1)/Delta,
xdd2 = -w2^2 x2 - (dV/dx2)/Delta, CI (x1,v1,x2,v2) = (A1,0,A2,0) avec
A1 = sgn*s(1+w2^2)/Delta, A2 = -sgn*s(1+w1^2)/Delta [equivalent au jet].
V = somme de monomes c x1^a x2^b ; variantes :
  full    : (g/p)(x1+x2)^p complet
  ablres  : monomes PORTEURS de l'harmonique resonante (2,1) retires —
            p=3 : g*x1^2 x2 ; p=5 : g*x1^4 x2 + 2g*x1^2 x2^3
  ablctrl : TEMOIN MIROIR (echange 1<->2), coefficients STRICTEMENT egaux,
            zero contenu (2,1) — p=3 : g*x1 x2^2 ;
            p=5 : g*x1 x2^4 + 2g*x1^3 x2^2
Limitation declaree : l'ablation monomiale retire aussi les harmoniques non
resonantes du meme monome ((0,±1),(2,-1) pour x1^2 x2) — la specificite est
portee par le TEMOIN MIROIR (affaiblissement de dominance apparie : memes
coefficients totaux, p.ex. 3g u^5 retires sur la diagonale des deux cotes a
p=5), pas par une chirurgie d'harmonique, impossible polynomialement.

**Statistiques** : R_res = s*_ablres/s*_full, R_ctrl = s*_ablctrl/s*_full,
**Q = R_res/R_ctrl** (l'affaiblissement de dominance se compense au premier
ordre dans le rapport). Protocole : w2=2.0, g=0.05, sgn=+1 gele (asym 5 %/0 %
au point resonant, M1/M3), RK4 dt=0.006, T=400, cap=1e4 sur max(|x1|,|x2|),
grille 48, 3 passes, auto-elargissement.

**Portes — chaque branche AVEC sa derivation (lecon E14) :**
- **P-M5a (p=3)** : Q >= 2.0 -> PLAFOND RESONANT CONFIRME [derivation :
  H-COURSE => le plafond cubique est la couche (2,1) ; son retrait specifique
  releve s* bien plus que le miroir]. Q <= 1.2 -> REFUTE [derivation : si le
  plafond n'etait pas resonant, retirer le monome porteur n'a aucun effet
  DIFFERENTIEL vs miroir]. Sinon NON CONCLUANT.
- **P-M5b (p=5)** : Q <= 1.3 -> PLAFOND NON-RESONANT [derivation :
  dominance-limite => les deux ablations appariees affaiblissent la dominance
  pareillement => Q ~ 1]. Q >= 2.0 -> RESONANT AUSSI A p=5 [derivation
  explicite de la branche : contredirait H-COURSE ; l'exception p=3 dans
  rho(T,K) devrait alors venir d'un AUTRE mecanisme — a ecrire tel quel].
  Sinon NON CONCLUANT.
- **P-M5c (p=3, detuning fin)** : s*(w2=2±0.1)/s*(2.0) >= 1.5 des deux cotes
  ET asymetrie des deux cotes <= x1.5 -> CREUX RESONANT LOCAL [derivation :
  couche de largeur finie => detuner exige plus d'amplitude pour l'atteindre].
  <= 1.2 d'un cote -> pas de creux local, lecture couche-fine refutee.
  Diagnostic non gele : demi-largeur du creux (delta ou s* double).
- **P-M5d (p=5, detuning fin)** : s*(2±0.1)/s*(2.0) <= 1.3 des deux cotes ->
  PLAT confirme [derivation : plafond non resonant => insensible au detuning
  fin ; coherent avec l'absence de collapse grossier, M3]. >= 1.5 ->
  sensibilite fine SANS collapse grossier [derivation : surprise a
  documenter — la resonance mordrait localement sans fixer K].
- **Gardes** : G1 pipeline integrateur-modes : s*_full(3, 2.0, g=0.05) vs
  M1 = 0.484 a ±8 % ; s*_full(5, 2.0) vs sP de out/m3_calib.json a ±8 %
  (si absent : note, pas de blocage). G2 invariance : K_p sur ablres a 2g,
  tolerance 10 %, reprise dense n=96 sinon EXCLU. G3 identite de force :
  la somme des monomes full doit redonner g(x1+x2)^(p-1) exactement (teste
  machine 1 avant livraison, RE-teste par assert au demarrage du run).

**Programme fige** : {3,5} x {full, ablres, ablctrl} a w2=2 (6) + ablres a 2g
(2) + detunings delta dans {±0.05, ±0.1} en full (8) = 16 recherches,
~5-8 min machine 2. Implementation : m5_course_v1.py (nom versionne, importe
OUT/s_target de bundle5, integrateur modes autonome), ecrit uniquement
out/m5_results.json (incremental). Gel jumeau dans le docstring, bloc hashe
de « PRE-ENREGISTREMENT M5 » a « === FIN DU GEL M5 === » inclus.
