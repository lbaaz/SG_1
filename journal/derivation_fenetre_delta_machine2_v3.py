#!/usr/bin/env python3
# -*- coding: ascii -*-
"""LA FENETRE DE delta -- derivation machine 2, v3. 12/09/2026 (soir).

CE QUE LA v3 CHANGE PAR RAPPORT A LA v2 (lot e3b707c589e9d1d8, relue par machine 1) :

  D-ACA-1, VERSE PAR MACHINE 1 ET ACCORDE SANS RESERVE -- LE CONTROLE A CONDITION
  LITTERALE `True` EST REMPLACE PAR UNE MESURE. La v2 imprimait en section 2
  `chk('donc ratio ... est PROPORTIONNEL a delta', True, ...)` : aucune issue ne pouvait
  le faire mordre, et il comptait dans le 21/21. C'est ma propre regle du 11/09 enfreinte
  dans ma propre feuille. La v3 le remplace par l'echelle MESUREE aux deux runs :
  `tau_CAP(delta_0) / tau_CAP(delta')` doit valoir `sqrt(1024) = 32`, donc `R` doit valoir
  1024 fois plus a delta'. L'issue qui ferait mordre est ecrivable : si `tau_CAP` n'allait
  pas en `sqrt(delta)`, le rapport ne serait pas 32 et l'echelle en 1/delta tomberait avec
  lui. La v2 N'EST PAS EDITEE (PB-1) ; elle reste la piece du lot e3b707c589e9d1d8.

  SA REMARQUE SUR LA SECTION 7, PRISE AUSSI (ce n'etait pas un defaut, c'est juste) : les
  comptes attendus (6, 3, 2, 4) ont ete lus sur les memes JSON avant d'etre ecrits en
  condition -- aiguille posee apres le compte. Ce sont des GELS DE NON-REGRESSION, pas des
  mesures, et ils le DISENT desormais dans leur intitule.

  LE COMPTE DE LA v2 SE LIT DONC : 21 lignes dont 20 controles. La v3 en porte 21 qui
  peuvent toutes mordre.

CE QUE LA v2 AVAIT CHANGE PAR RAPPORT A LA v1 (lot 7883311c6e363b02) :
...
# D-ACA-1 : ici la v2 imprimait un chk a condition `True`. L'echelle qu'il ENONCAIT est
# MESURABLE aux deux runs -- tau_CAP ~ sqrt(delta), donc R ~ 1/delta, donc (avec e
# invariante) ratio ~ delta. Mordrait si tau_CAP n'allait pas en sqrt(delta).
ech = [REF[k]['tau_CAP'] / PRE[k]['tau_CAP'] for k in sorted(PRE)]
pire_e = max(abs(q - 32.0) / 32.0 for q in ech)
chk('tau_CAP(delta_0)/tau_CAP(delta-prime) == sqrt(1024) aux 9 points', pire_e < 1e-12,
    'ecart max %.2e ; rapport R mesure %.6f' % (pire_e, max(ech) ** 2))
chk('donc ratio = e/(C x plancher) est PROPORTIONNEL a delta',
    pire_e < 1e-12 and 0.8 < lo and hi < 1.25,
    'R x %.0f et e invariante -> ratio x %.0f' % (max(ech) ** 2, max(ech) ** 2))
...
# =====================================================================
# Les quatre comptes attendus (6, 3, 2, 4) ont ete LUS sur ces memes JSON avant d'etre
# ecrits en condition -- aiguille posee apres le compte (remarque de machine 1, juste).
# Ce sont des GELS DE NON-REGRESSION : ils attrapent un changement de source, ils ne
# MESURENT pas le compte. Ils le disent dans leur intitule (GEL NR).
...
chk('GEL NR -- au bit, elles different sur 6 cellules', len(dif_n) == 6,
    ','.join(dif_n))
chk('GEL NR -- a 3 decimales, sur 3 : le compte de l erratum', len(dif_3) == 3,
    ','.join(dif_3))
...
chk('GEL NR -- parmi les 3 GROSSIERES, 2 sont LUES', len(lues_g) == 2, ','.join(lues_g))
chk('GEL NR -- parmi les 6 AU BIT, 4 sont LUES', len(lues_b) == 4, ','.join(lues_b))
