# PRE-ENREGISTREMENT alpha v3 -- L'EXPOSANT DU PROFIL D'EXPLOSION EST-IL 4 u_p ?
# BROUILLON MACHINE 2 -- DEVIENT GEL A LA CERTIFICATION MACHINE 1

Fichier    : alpha_pre_enregistrement_v3.md
...
4.4 Branches. p PAIR : les deux branches explosent symetriquement
(parite acquise en M11) -- la manche joue sgn = +1 et DECLARE que -1 en
est l'image. p IMPAIR : x^(p-1) > 0, une seule branche pour g > 0 -- la
question de l'autre branche est SANS OBJET.
...
=======================================================================
    ...
    G-dt    dt_2 et dt_2/2, MEME trajectoire : meme alpha **a la
            tolerance de 10.1bis, tol_G_dt, QUI NE CONTIENT PAS l'ecart
            teste**. Sinon **NON CONCLUANT DE RESOLUTION**.
    G-k     k = 2 et k = 4, MEME trajectoire : meme alpha **a tol_G_k
            (10.1bis)**.
...
10.1bis **UNE GARDE NE SE COMPARE PAS A UNE TOLERANCE QUI LA
CONTIENT.** La tolerance de 10.1 est le maximum de trois quantites, dont
les ecarts que G-dt et G-k testent : comparer l'un a leur maximum ne peut
jamais mordre. Chaque garde se compare donc a la tolerance PRIVEE de son
propre ecart :

    tol_G_dt = max( (ii) l'ecart G-k , (iii) la dispersion locale )
    tol_G_k  = max( (i)  l'ecart G-dt, (iii) la dispersion locale )

    G-dt MORD si l'ecart G-dt > tol_G_dt ; G-k de meme.
    Les deux se plafonnent comme 10.2, et **tol_G_dt / (8/15) et
    tol_G_k / (8/15) se CONSIGNENT**, passe ou non.

Aucune quantite neuve n'est introduite : ce sont les memes trois
composantes, prises deux a deux. La tolerance de 10.1, elle, reste
inchangee et continue de servir a P-alpha, a G-s et a G-w2 -- dont les
ecarts, eux, n'entrent PAS dans son maximum.
...
-- FIN alpha_pre_enregistrement_v3 --
