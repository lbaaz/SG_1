# PRE-ENREGISTREMENT constante A v3 -- LA FENETRE DESCEND JUSQU'A CE QUE
...
CE QUE LA v3 CHANGE PAR RAPPORT A LA v2 -- TROIS ENTREES, NOMMEES
             La v2 (69dd3c6a1e8a21f4, 24038 o) est REMPLACEE, non
             editee. Sources : transmission machine 2 "compte 2s"
             (POUR_MACHINE1_constante_A_v2_compte_2s_v1.md,
             67a4be1d02b89748, 3279 o) et passe de transposition du
             temoin (note_machine2_transposition_temoin_v7_delta_prime_
             v1.md, a6f2f6e27075cda9, 10827 o), chacune RE-DERIVEE par
             machine 1 avant reprise :
             1. COMPTE 2s (fond) -- 4.6/4.7 : "620 exactement" etait un
                NOMINAL affirme, pas un compte -- l'etage 2s part de la
                grille de phase 1, jusqu'a un dt_1 avant depart_2s, et
                compte 658 / 631 / 646 selon w2. Un compte inscrit se
                COMPTE ; la v3 ecrit nominaux, comptes mesures et
                intervalles derives pour TOUS les etages, des deux
                cotes du seuil. Deuxieme assertion fausse de brouillon
                machine 1 de la journee (apres 4.7-v1) : a verser a
                l'acte, numeros au depot (E18).
             2. PORTE (fond) -- 5 : la passe de transposition du v7 est
                FAITE ; W-bascule est INJOUABLE a delta' (bloquant) et
                trois autres entrees sont dues : un temoin v8 est du,
                plume machine 2, arbitrage operateur. La section 5 est
                reecrite sur la v8 ; l'exigence du volet A sur la
                remise d'etat (voie A ou equivalent) est ECRITE ; le
                controle de reproductibilite T-1/T-1b/T-3 au bit contre
                le run 85 entre dans la porte.
             3. REGLE 15 (forme, mesure machine 1) -- 4.7 : k tau_dom'
                /400 et tau_CAP'/M, mathematiquement egaux (1/200 en
                exact), DIFFERENT au bit en flottant selon l'ordre
                d'evaluation. L'egalite du pas 2b' a sa borne se dit en
                arithmetique EXACTE et ne se teste jamais au bit --
                l'"egalite d'objet" que la v2 aurait laisse croire ici
                etait une mine.
             RIEN d'autre ne change : delta' et sa derivation (3), les
             tables 4.1-4.5, la porte du plancher (7), la cascade (8),
             la lecture L-desc et sa puissance (9), les comptes de
             volet (11), les sections 0-2 et 12.
...
28998 o) reste la base. NE CHANGENT PAS, et valent ici par citation :
    ...
    5.6    temoin de lignee : 27/27 au booleen et a l'indice, ou le lien
           n'est pas etabli
    5.7    la sortie : serie de la fenetre, etat complet a chaque
           bascule et a chaque depart d'etage, journal de phase 1,
...
seuil), une porte de qualification sur un temoin v8, une porte de
plancher, une lecture consignee a portee declaree.
...
pas de la grille, jusqu'a UN dt_1 AVANT depart_2s -- son compte n'est
PAS 620 :

    t_start = floor(depart_2s / dt_1) x dt_1     [etat consigne, 5.7]
...
REGLE 15, ECRITE (mesure machine 1, 28/08) : k tau_dom'/400 et
tau_CAP'/M sont EGAUX en arithmetique exacte (k/400 = r/M = 1/200,
Fraction) ; en flottant, les deux expressions DIFFERENT au bit selon
l'ordre d'evaluation. L'egalite du pas 2b' a sa borne de 4.2 se
DECLARE en exact et ne se teste JAMAIS au bit ; aucune garde de ce gel
ne compare deux derivations flottantes d'une meme grandeur.
...
=======================================================================
5. LA PORTE DE QUALIFICATION (D-alpha-7 transposee) -- SUR LE TEMOIN v8
=======================================================================

La passe de transposition du v7 au reglage prime est FAITE (note
machine 2 a6f2f6e27075cda9, 28/08 ; perimetre : les 84 lignes du v7
portant un token en delta, enumerees). Son resultat, re-derive machine
1 sur le texte du v7 (8b083e9f109b5a8e, lignes 444, 571/631, 179,
192/572) :

    -- T-1, T-1b, T-3 sont SANS delta ; le coeur de T-2 part d'un etat
       analytique EXACT et ses grandeurs sont auto-similaires (380 pas
       d'etage, 180 de fenetre, tol_int sur ln(1/r)) : ils transposent.
    -- UNE GARDE TOMBE : W-bascule est INJOUABLE a delta' -- sa phase
       grossiere, de portee 2 tau_dom', ne contient AUCUN pas complet
       de dt_1 (0.350 a 0.521 pas). Troisieme instance de la famille
       "garde injouable a compte gele". Elle porte 9 des 41 runs.
    -- Trois autres entrees sont dues : la raideur de 5.2 en 1/delta
       (x1024 au reglage prime, et le point d'evaluation dans la
       fenetre manque au texte) ; 8 et 9 en contradiction sur
       W-bascule (le "sans" de 9 est faux) ; 3 herite delta du gel
       alpha v2 PERIME (35a70834b2a34514) au lieu de la v5.

**Un temoin v8 est du** -- plume machine 2, arbitrage operateur
(chemin E43). Numeros locaux a la famille du temoin, au depot (E18).

CE QUE LA PORTE DE CE GEL EXIGE DE LA v8 -- et c'est le volet A qui
parle : la remise d'etat aux bascules est PRECISEMENT l'organe que le
reglage prime complique (trois etages au lieu de deux, des deux cotes
du seuil). La porte n'est satisfaite que si cette remise est QUALIFIEE
par une garde JOUEE : W-bascule par etages (voie A de la passe -- la
phase grossiere descend g1 puis g2, la chaine complete est qualifiee,
les 41 runs conserves), ou equivalent declare. Une v8 en voie B
(W-bascule non jouee, attendus 32) laisserait cet organe sans garde :
elle rendrait la porte de CE gel INSATISFAITE et le volet A ne se
jouerait pas. La voie se tranche a l'arbitrage operateur ; l'exigence
du volet A est ecrite ici pour qu'elle se tranche en face.

La v8 porte aussi le CONTROLE DE REPRODUCTIBILITE de la passe : T-1,
T-1b, T-3, sans delta, se rejouent A L'IDENTIQUE et leurs cles se
comparent a celles du run 85 (644240dc894c2733) par ENUMERATION a
profondeur declaree ; ecart sur une cle non exempte -> NON CONCLUANT
D'INSTRUMENT, avant toute lecture de verdict. Ce n'est pas une
qualification neuve et ne se presente pas comme telle (regle 1 du
28/08) : c'est la mesure de reproductibilite que N-70 exige.

Aucun run du volet A n'est opposable avant le verdict REGLAGE QUALIFIE
(ou PASSE) du volet T sous la **v8 CERTIFIEE**, jouee sous LE MEME
reglage (delta', r, M, k, la regle du pas par etage) et sur le meme
instrument ; sa prediction se depose avant le run par ENUMERATION des
cles (N-70). Le verdict du volet T est lu par machine 2 sur BOCAL4, au
...
Part de banc_qualification_machine1_v3.py (5fae2a8c94cf8685). Prend :
    ...
    (iii) les etages 2a/2b sur-seuil (4.2) ET les etages 2s/2b'
          sous-seuil ancres en t (4.7), avec leurs sorties 5.7 (etat
          complet a CHAQUE bascule et a CHAQUE depart d'etage,
          empreintes) ; AUCUN compte d'etage n'est asserte dans le
          code : les comptes se MESURENT et se consignent, toute garde
          de compte se compare aux intervalles derives de 4.6/4.7 ;
...
v8, aux grandeurs primees, en forme derivee (41 en voie A ; toute
autre valeur porte sa derivation). Chaque compte en forme derivee,
jamais recopie.
...
-- FIN constante_A_pre_enregistrement_v3 --
