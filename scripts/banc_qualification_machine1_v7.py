...
VERSION = "banc_qualification_machine1_v7"
...
def banc(registre, mod, sortie):
    ...
    scenario("G34 D-I-5/6 : identite 0 ulp ; x''+1 et D''+1 PASSENT (bornes 2 / 2) ; x''+3 et D''+3 MORDENT ; tirage+1 MORD",
             c_id["verdict"] == "PASSE" and c_id["max_ulp_Dpp"] == 0.0
             and c_x1["verdict"] == "PASSE" and 0 < c_x1["max_ulp_xpp"] <= 2
...
def comparer_champs_ulp(texte_a, texte_b):
    ...
    if u_x > 2:
        fautes.append("x'' %.1f ulp > 2 (pow libm + arrondi)" % u_x)
    if u_D > 2:
        fautes.append("D'' %.1f ulp > 2 (re-association)" % u_D)
    ...
    return {"verdict": v, "n": len(A), "tirage_au_bit": tir,
            ...
            "resume": "%s -- tirage %s ; x'' max %.1f ulp (%d ecarts) ; D'' max %.1f ulp (%d ecarts) ; bornes 2 / 2%s"
