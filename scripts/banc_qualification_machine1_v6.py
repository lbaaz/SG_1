...
VERSION = "banc_qualification_machine1_v6"
...
def executer_temoin(registre, sortie, mod, prevol=None, ref_9bis=None):
    ...
    if CHAMP_COMPARE:
        ...
        L["W_transcription_nombres"] = {"verdict": "A COMPARER",
            "resume": "seconde transcription non fournie -- fichier LIVRE, la comparaison se joue sur les nombres (bornes 0 / 2 ulp)"}
...
def banc(registre, mod, sortie):
    ...
    # -- G34 : D-I-5 (3), la garde SUR LES NOMBRES -- bornes 0 / 2 ulp
    ...
    scenario("G34 D-I-5 : identite PASSE 0 ulp ; D''+1 ulp PASSE (borne 2) ; D''+3 MORD ; x''+1 MORD ; tirage+1 MORD",
             c_id["verdict"] == "PASSE" and c_id["max_ulp_Dpp"] == 0.0
             and c_D1["verdict"] == "PASSE" and 0 < c_D1["max_ulp_Dpp"] <= 2
             and c_D3["verdict"] == "MORD" and "D''" in c_D3["resume"]
             and c_x1["verdict"] == "MORD" and "x''" in c_x1["resume"]
             and c_t1["verdict"] == "MORD" and "TIRAGE" in c_t1["resume"],
             c_D3["resume"][:80], gardes=("W-transcription",))
...
def comparer_champs_ulp(texte_a, texte_b):
    """D-I-5 (3) : la garde se lit SUR LES NOMBRES, bornes DERIVEES --
    TIRAGE au bit ; x'' : 0 ulp (chaine +, -, x IEEE-exacte) ;
    D'' : <= 2 ulp (re-association d'un produit a trois facteurs ;
    mesure machine 2, note 5bis, versee). Jamais un verdict vide."""
    def _donnees(t):
    ...
    if u_x > 0:
        fautes.append("x'' %.1f ulp > 0 (chaine exacte)" % u_x)
    if u_D > 2:
        fautes.append("D'' %.1f ulp > 2 (borne d'association)" % u_D)
    ...
    return {"verdict": v, "n": len(A), "tirage_au_bit": tir,
            ...
            "resume": "%s -- tirage %s ; x'' max %.1f ulp (%d ecarts) ; D'' max %.1f ulp (%d ecarts) ; bornes 0 / 2%s"
                      % (v, "BIT" if tir else "ECART", u_x, n_x, u_D, n_D,
...
def main():
    ...
    ap.add_argument("--champ-compare", default=None, help="fichier champ (4096 x 4) d'une SECONDE transcription : comparaison SUR LES NOMBRES, bornes derivees 0 / 2 ulp (D-I-5)")
    a = ap.parse_args()
