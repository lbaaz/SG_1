...
"""
# =====================================================================
# m15_site83_v2.py -- MANCHE M15 (P1-b, site 8/3), machine 1, 2026-08-08
# v2, apres certification du script (machine 2, a reception 4a0cfc58...)
# et run reel (96d78407...). QUATRE changements, tous declares :
#   D-2  cote du pas = convention (f) de la ligne (frag), plus de sP
#        force a p=4 (effet nul demontre : barre/plancher 1/1619) ;
#   D-3  C2 decide par appartenance EXACTE en Fraction (regle 11),
#        plus de cle formatee (effet nul : centiemes exacts) ;
#   B'   scenario "Bp" (nextafter vers -inf) : branche frag=-1 a p=4
#        TRAVERSEE et asseree ;
#   +    consignation X_survivants (E, S57, S4, c5, c7 par survivant,
#        meme sous plancher manque) -- aucune porte n'y touche.
# Le gel jumeau reste la v3 CERTIFIEE e41f4da3, INCHANGEE : l'erratum
# "grossiere mordue" vit au registre (le script implemente deja la
# definition corrigee) ; portage du texte au gel v4. Cette v2 ne
# touche PAS au run execute 96d78407 : version de lignee.
...
def criterer(survivants, X, B, K, largeur_prog=True):
    ...
    qmap = {"%.2f" % float(q): q for q in interieurs}
    xM_cle = max(lignes, key=lambda c: abs(lignes[c]["res_E"]))
    xM = lignes[xM_cle]
    xM_q = qmap[xM_cle]
    dossier["x_M"] = float(xM_q)
    C1 = abs(xM["res_E"]) > xM["seuil_E"]
    C2 = xM_q in (PROCHEg, PROCHEd)      # appartenance EXACTE (regle 11, D-3)
...
SCENARIOS_PREVOL = {
    ...
    "Bp": {"morts": {}, "g8_casse_moins": True,
           "attend": "B-prime : G8a ECHEC par nextafter vers -inf ; "
                     "branche frag = -1 a p=4 TRAVERSEE (D-2)"},
...
def fabriquer_factice_m15(cible_g1p, scenario):
    ...
    def s_de(p, w, sgn):
        ...
        if sc.get("g8_casse_moins") and p == 4 and sgn < 0:
            s = math.nextafter(s, -math.inf)
        return s
...
def verifier_scenario(sc, res, dossier, sig, cverdict, g8_echec, P):
    ...
    if sc == "A":
        ...
        assert g8_echec, "Bp: G8 aurait du echouer"
        assert P.CPT["recherches"] + P.CPT["sautees"] == rech_attendues(True), \
            "Bp: comptes non derives sous (f) integrale"
        frags4 = [v.get("frag") for c, v in res["resultats"]["carte"].items()
                  if c.startswith("4|")]
        assert -1 in frags4, "Bp: la branche frag=-1 a p=4 n'est pas traversee"
    elif sc == "C":
...
def main():
    ...
    for w in survivants:
        ...
        for p in DEGRES:
            v = carte[P.cle(p, w)]
            sfs[p] = v["sF"]
            cote = "sP" if v.get("frag") == 1 else "sM"   # D-2 : le pas suit
            pas[p] = (P.pas_final(v[cote]["note"]), v["sF"])  # la convention (f)
        X[w] = x_du_point(sfs[4], sfs[5], sfs[7])
        B[w] = {nom: b_sigma(pas, nom) for nom in ("E", "S57", "S4")}
    dossier = criterer(survivants, X, B, K)
    # v2 : CONSIGNATION (aucune porte) -- les observables par survivant,
    # meme sous plancher manque ; E(w) n'est plus a re-deriver a la main.
    dossier["X_survivants"] = {"%.2f" % w: X[w] for w in survivants}
