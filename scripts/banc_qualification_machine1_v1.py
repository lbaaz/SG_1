...
# -*- coding: ascii -*-
"""
banc_qualification_machine1_v1.py -- L'INSTRUMENT DES DEUX BANCS DE QUALIFICATION
================================================================================
Version 1. Redaction MACHINE 1 (Claude) -- la main est dans le nom (N-65).
...
VERSION = "banc_qualification_machine1_v1"
...
def cascade_temoin(L):
    ...
    pts = T2["points"]
    mordent = [k for k in pts if "MORD" in (pts[k]["W_pas"], pts[k]["W_plancher"], pts[k]["W_bascule"])]
    if mordent:
        return "NON CONCLUANT D'INTEGRATEUR", "branche 4 : W-pas/W-plancher/W-bascule MORD en %s" % mordent
...
def executer_temoin(registre, sortie, mod, prevol=None):
    ...
    L["W_comptes"] = "PASSE" if compteur["comptes"] + compteur["sautes"] == n_att else "MORD"
    JRN("W-comptes", "comptes %d + sautes %d == attendus %d -> %s" % (compteur["comptes"], compteur["sautes"], n_att, L["W_comptes"]))
    if L["W_comptes"] == "MORD" and L["verdict"] != "BANC NON JOUE":
        L["verdict"], L["branche"] = "BANC NON JOUE", "W-comptes MORD"
...
def cascade_alpha(L):
    ...
    if any(not deg[p]["resolution_ok"] for p in deg):
        return prefixe + "NON CONCLUANT DE RESOLUTION", "branche 2 : G-dt, G-k ou plafond 10.2 (tol > 2/15)"
    ...
    if any(deg[p]["G_s_mord"] or deg[p]["G_w2_mord"] for p in deg):
        return prefixe + "REFUTE", "branche 4 : G-s ou G-w2 MORD (alpha depend de s ou de w2)"
...
def lire_alpha(plan, gdt, gk, seuil, lignee, e_ln10_max):
    ...
    L = {"plan": plan, "G_dt": gdt, "G_k": gk, "seuil": {}, "degres": {},
...
def executer_alpha(registre, sortie, mod, porte, prevol=None):
    ...
    L["comptes"] = dict(compteur); L["attendus"] = attendu; L["attendus_total"] = n_att
    L["G_comptes"] = "PASSE" if compteur["comptes"] + compteur["sautes"] == n_att else "MORD"
    JRN("G-comptes", "comptes %d + sautes %d == attendus %d -> %s" % (compteur["comptes"], compteur["sautes"], n_att, L["G_comptes"]))
    if L["G_comptes"] == "MORD":
        L["verdict"], L["branche"] = "MANCHE NON JOUEE", "G-comptes MORD"
    JRN("VERDICT", "%s -- %s" % (L["verdict"], L["branche"]))
    return L
...
class FlotSynthetique(object):
    ...
    def __init__(self, etat, caps, dt=DT1, regime="lineaire", v=0.5, t_star=100.0, gigue=0.3):
...
def integrer_synthetique(loi, v=0.512, theta=0.9):
    ...
    def fabrique(cap, tm):
        th = cap / (v * tm) if loi == "lineaire" else theta
...
class SynthAlpha(object):
    ...
    def __call__(self, w2, s, p, mode, x_b=None, t_fin=None, **kw):
        ...
        if mode == "bascule":
            ts = self.t_star_de(w2, p, s); tb = ts - K_BASC * tau_dom(w2) if x_b >= x_bascule(p, w2, K_BASC) * 0.999 else ts - K_GARDE * tau_dom(w2)
            n = int(round(tb / DT1))
...
def banc(registre, mod, sortie):
    ...
    scenario("B1 cascade : faux blow-up -> REGLAGE REFUTE branche 3", v == "REGLAGE REFUTE" and "saturation" in b, v + " / " + b)
    ...
    scenario("B2 cascade : mirage -> REGLAGE REFUTE branche 3 (W-mirage)", v == "REGLAGE REFUTE" and "mirage" in b, v + " / " + b)
    ...
    scenario("B3a transcription fausse (NPASSES=2) : controle positif MORD", not ok_f, det_f)
    ...
    scenario("B3d physique corrompue (exposant p) contre le moteur : G-lignee ECART", not rec["ok"],
             "booleen %s indice %s" % (rec["booleen_identique"], rec["indice_identique"]))
    ...
    scenario("B4d sous le seuil, un faux blow-up -> G-seuil MORD -> INSTRUMENT REFUTE", v == "INSTRUMENT REFUTE", v + " / " + b)
    v, b = cascade_alpha(L_s)
    scenario("B4e alpha dependant de s -> G-s MORD -> REFUTE branche 4", v == "REFUTE" and "G-s" in b, v + " / " + b)
    # B5 -- le compte : comptes + sautes != attendus doit MORD
    scenario("B5 W-comptes : 38 + 0 != 39 -> MORD", (38 + 0 == sum(compte_attendu_temoin().values())) is False)
    ...
    JRN("BANC", "bilan %d/%d scenarios mordent" % (n_ok, len(S)))
    return n_ok == len(S), S
